				...
				<tbody>
					...
					?>
						<tr>
							...
							<td><p class="m-0 truncate-1 fbx"><?= $row['description'] ?></p></td>
							<td class="text-center">
                                <?php if($row['status'] == 1): ?>
                                    <span class="fbx badge badge-success px-3 rounded-pill">نشط</span>
                                <?php else: ?>
                                    <span class="fbx badge badge-danger px-3 rounded-pill">غير نشط</span>
                                <?php endif; ?>
                            </td>
							<td align="center">
...
<script>
	$(document).ready(function(){
		...
		$('#create_new').click(function(){
			uni_modal("<i class='fa fa-plus'></i> إضافة فئة جديدة","categories/manage_category.php")
		})
		$('.view_data').click(function(){
			uni_modal("<i class='fa fa-bars'></i> تفاصيل الفئة","categories/view_category.php?id="+$(this).attr('data-id'))
		})
		$('.edit_data').click(function(){
			uni_modal("<i class='fa fa-edit'></i> تحديث تفاصيل الفئة","categories/manage_category.php?id="+$(this).attr('data-id'))
		})
		$('.table').dataTable({
			columnDefs: [
