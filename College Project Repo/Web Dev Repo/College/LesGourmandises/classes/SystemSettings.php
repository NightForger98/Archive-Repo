...
class SystemSettings extends DBConnection{
	...
	function update_dollar_rate($new_rate) {
        $new_rate = $this->conn->real_escape_string($new_rate); // Sanitize input
        $meta_field = 'dollar_rate';
        $qry = $this->conn->query("SELECT COUNT(*) as count FROM system_info WHERE meta_field = '{$meta_field}'");
        $row = $qry->fetch_assoc();

        if ($row['count'] > 0) {
            // Update existing rate
            $sql = "UPDATE system_info SET meta_value = '{$new_rate}' WHERE meta_field = '{$meta_field}'";
        } else {
            // Insert new rate
            $sql = "INSERT INTO system_info (meta_field, meta_value) VALUES ('{$meta_field}', '{$new_rate}')";
        }
        
        $result = $this->conn->query($sql);
        if ($result) {
            $_SESSION['system_info'][$meta_field] = $new_rate; // Update session data
            return json_encode(['status' => 'success', 'msg' => 'Dollar rate updated successfully.']);
        } else {
            return json_encode(['status' => 'failed', 'msg' => 'Failed to update dollar rate.']);
        }
    }
